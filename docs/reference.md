...
- **Admin login**: `admin` / `password` (the Playground CLI default). In development mode the staged `playground-dev-login.php` mu-plugin auto-submits any plain GET of the wp-login form as `admin` — the CLI's own auto-login is disabled there, because it re-logs-in every request that lacks its marker cookie (cookie-less curl probes and health checks redirect-loop forever, each pass writing a session). Deliberate flows keep the form: POSTs, logouts (`?loggedout`), password resets, and an explicit `wp-login.php?action=login` (to sign in as someone else). The storefront stays guest-browsable. Auto-login is **local-only**: it requires a loopback Host (direct http and `--https` qualify), so requests through a `--tunnel` URL always get the normal form. Dev sites are throwaway either way; keep production keys out.
- **Guest mode** (development mode): `?krokedil-guest=1` on any local URL — or **Browse as guest** in the admin bar — logs you out and sets a 12-hour `krokedil_pg_guest` cookie that stands both this auto-login and Playground's own down, so the browser stays a visitor. It exists because WordPress builds front-end login links as plain `wp-login.php?redirect_to=…` GETs, which auto-login signs in: without it, one click on a comment form or the Meta widget ends a logged-out test. `?krokedil-guest=0` (or letting the cookie expire) restores auto-login; the toggle is per browser and ignored for non-local requests. Entering guest mode clears the WooCommerce cart with the session. There is no seeded customer account — guest mode covers *logged-out* testing, not logged-in-customer testing.
- **Tunnel guard** (all modes, active only while `--tunnel` runs): `playground-tunnel-guard.php` refuses the default `password` for requests that didn't come from your machine and accepts only that run's tunnel password — printed with the public URL, or `KROKEDIL_PG_TUNNEL_PASS` when you set it. It also blocks Playground's built-in cookie-marker auto-login for those requests, so no public request can ever be authenticated without a password. Local logins, the dev auto-login above and `screenshots` are unaffected; the storefront and webhook callbacks stay public. See [README — Tunnel logins](../README.md#tunnel-logins-need-the-run-password).
